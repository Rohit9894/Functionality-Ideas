...
function getData(filter) {
  if (!filter) {
    ...
    return fetch(`https://restcountries.com/v3.1/region/${filter}`).then(
      (res) => res.json()
    );
  }
}
      ...
      >
        {data.map((item) => (
          ...
          >
            ...
            <h3>{item?.name?.nativeName?.ara?.official}</h3>
            <MoreDetails
              key={item.population}
              data={item}
              //   native={item?.name?.nativeName?.ara?.official}
              //   currency={item?.currencies?.MRU?.name}
              //   subRegion={item.subRegion}
              //   langauge={item.langauges.ara}
